...
class ProductController extends Controller
{
    ...
    public function storeVariant()
    {
        try {
            ...
            $dataFile = $_FILES;
            // $dataPost = ['product_id' => $_GET['id'], 'color_id' => $_POST['color'], 'size_id' => $_POST['size'], 'base_price' => $_POST['base_price'], 'sale_price' => $_POST['sale_price'], 'quantity' => $_POST['quantity'], 'image_main' => $_FILES['image_main']['name']];
    ...
    {
        try {
            $dataVariant = ['product_id' => $_GET['product_id'], 'color_id' => $_POST['color'], 'size_id' => $_POST['size'], 'base_price' => $_POST['base_price'], 'sale_price' => $_POST['sale_price'], 'quantity' => $_POST['quantity']];

            if ($_FILES['image_main']['size'] > 0) {
                $dataVariant['image_main'] = $_FILES['image_main']['name'];
                move_uploaded_file($_FILES['image_main']['tmp_name'], "uploads/" . $_FILES['image_main']['name']);
            }


            $count = $this->variant->update($dataVariant, "variant_id = :id", ["id" => $_GET['id']]);
            $imageLenght = count($_FILES['image']['name']);
            if ($imageLenght != 3) {
                $_SESSION['error']['image'] = "Vui lòng chọn đủ 3 hình ảnh";
            } else {
                for ($i = 0; $i < $imageLenght; $i++) {
                    move_uploaded_file($_FILES['image']['tmp_name'][$i], "uploads/" . $_FILES['image']['name'][$i]);
                    $imageNew = $_FILES['image']['name'][$i];
                ...
                $rowCount = $this->variant->delete2("image_variant", "variant_id = :id", ["id" => $_GET['id']]);
                $countImage = count($imageIdNew);
                for ($i = 0; $i < $countImage; $i++) {
                    $this->variant->insert2(['variant_id' => $_GET['id'], 'image_id' => $imageIdNew[$i]], "image_variant");
                }
            }
            if (!empty($_SESSION['error'])) {
                throw new Exception("Dữ liệu lỗi");
                header("location:?role=admin&controller=product&action=editVariant&id=" . $_GET['id'] . "&product_id=" . $_GET['product_id']);
                die;
            }
        } catch (\Throwable $th) {
            $_SESSION['success'] = false;
